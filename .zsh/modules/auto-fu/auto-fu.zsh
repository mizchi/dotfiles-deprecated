...
# TODO: cp x /usr/loc
# TODO: region_highlight vs afu-able-p ขช nil
...
# v0.0.1.8.3
# in afu+complete-word PAGER=<TAB> ขอ PAGER=PAGER= bug fix.
...
afu+complete-word () {
  ...
  if ((afu_in_p == 1)); then
    ...
    case $LBUFFER[-1] in
      (=) # --prefix= ขอ complete-word again for `magic-space'ing the suffix
        { # TODO: this may not be accurate.
          local x="${${(@z)LBUFFER}[-1]}"
          [[ "$x" == -* ]] && zle complete-word && return
        };;
      (/) # path-ish  ขอ propagate auto-fu if it could be
      ...
      (,) # glob-ish  ขอ activate the `complete-word''s suffix
        BUFFER="$buffer_cur"; zle complete-word;
        return
        ;;
    esac
    (( $_lastcomp[nmatches]  > 1 )) &&
      # many matches ขอ complete-word again to enter the menuselect
      zle complete-word
    (( $_lastcomp[nmatches] == 1 )) &&
      # exact match  ขอ flag not using _oldlist for the next complete-word
